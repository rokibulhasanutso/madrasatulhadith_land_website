...
import { monthlyExamSubjectName } from "../../staticData/monthlyRoutineData";
import { classCode } from "../../staticData/classSubjectData";
import supabase from "../../supabase/config";
import { enToBnNumber } from "../../utils/utils";
import { BadgeAlert, BadgeCheck, Loader, X } from "lucide-react";
...
const getExistingMark = (data, subjectKey, mainDataUpdate) => {
  // console.log(mainDataUpdate.data, data);
  return (
    mainDataUpdate?.data.find((val) => val?.student_id === data?.id)?.[
      subjectKey
    ] ||
    data?.[subjectKey] ||
    "00"
...
const MonthlyInsertResult = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { store, studentResultData, refresh } = useDataStore();
  ...
  const subjectLabel = monthlyExamSubjectName[subjectKey];
  ...
  const handleUpdateBatchResult = async () => {
    ...
    console.log(mainDataUpdate.data);
                ...
                <div className="flex justify-between">
                  ...
                  <p>
                    ...
                    <span className="">
                      {enToBnNumber(
                        getExistingMark(
                          studentResultData?.find(
                            (item) => item.student_id === data.id
                          ) || data,
                          subjectKey,
                          mainDataUpdate
                        )
          ...
          <div className="relative h-full flex flex-col justify-center">
            <div className="flex justify-between items-center p-1.5 border-b border-slate-300">
              ...
              <h1 className="text-xl text-center font-semibold ms-2.5">
                {`${modalFullData.name} (${enToBnNumber(modalFullData.roll)})`}
              </h1>
...
export default MonthlyInsertResult;
